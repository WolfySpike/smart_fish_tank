...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	uint16_t XOffset = 0;
	uint16_t pIndex;
	uint8_t HasMultiByteChar = 0;

	/* 8x16模式下，若字符串包含中文，则英文和空格按6x8绘制，中文保持16x16 */
	if (FontSize == OLED_8X16)
	{
		uint16_t j = 0;
		while (String[j] != '\0')
		{
			if ((String[j] & 0x80) != 0x00)
			{
				HasMultiByteChar = 1;
				break;
			}
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		if (CharLength == 1)	//如果是单字节字符
		{
			...
			if ((FontSize == OLED_8X16) && (HasMultiByteChar != 0U))
			{
				/* 混排时，ASCII/空格使用6x8并垂直居中 */
				OLED_ShowChar((int16_t)(X + XOffset), (int16_t)(Y + 4), SingleChar[0], OLED_6X8);
				XOffset += OLED_6X8;
			}
			else
			{
